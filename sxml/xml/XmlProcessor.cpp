...
XmlProcessor::XmlProcessor(TrivialLexer& lexer_, const char32_t* contentStart_, const char32_t* contentEnd_, XmlContentHandler* contentHandler_) :
    lexer(lexer_), contentStart(contentStart_), contentEnd(contentEnd_), contentHandler(contentHandler_), attValue()
{
...
std::string XmlProcessor::GetErrorLocationStr(const std::string& systemId, const soulng::lexer::Span& span) const 
{
    ...
    int errorColumn = GetErrorColumn(token.match.begin - contentStart);
    return "error in '" + systemId + "' at line " + std::to_string(span.line) + " column " + std::to_string(errorColumn) + " (index " + std::to_string(token.match.begin - contentStart) + ")";
}
